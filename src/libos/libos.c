...
typedef enum {
  ...
  PARAM_DRIVER, PARAM_LAUNCHER, PARAM_OSVERSION, PARAM_DEVICEID, PARAM_COMPANYID, PARAM_LOGTRAP, PARAM_LOGSCR,
  PARAM_IMMEDIATE, PARAM_OPENGL, PARAM_HACKS
} param_id_t;
...
static int libos_start(int pe) {
  ...
  if ((data = sys_calloc(1, sizeof(libos_t))) != NULL) {
    ...
    debug(DEBUG_INFO, PUMPKINOS, "mode %d, screen dimensions %dx%d, depth %d, host depth %d", data->mode, data->width, data->height, data->depth, data->hdepth);

    if (data->width > 0 && data->height > 0 && (data->hdepth == 16 || data->hdepth == 32)) {
      if (data->abgr) data->hdepth = 32;
      if (thread_needs_run()) {
