...
{
    DG_debug(DEBUG_INFO, "%s", msg);
}

// haleyjd: moved up, removed WATCOMC code
void CleanExit(void)
{
    DG_debug(DEBUG_INFO, "Exited from HERETIC");
    exit(1);
}
...
//
    ...
    D_BindVariables();
    M_SetConfigFilenames("heretic.cfg", NULL);
    M_LoadDefaults();
    ...
    tprintf("NET_Init: Init network subsystem", 1);
    NET_Init ();
    ...
    tprintf(DEH_String("MN_Init: Init menu system"), 1);
    MN_Init();
    ...
    tprintf(DEH_String("P_Init: Init Playloop state"), 1);
    hprintf(DEH_String("Init game engine."));
    P_Init();
    IncThermo();

    tprintf(DEH_String("I_Init: Setting up machine state"), 1);
    //I_CheckIsScreensaver();
    //I_InitJoystick();
    IncThermo();

    tprintf(DEH_String("S_Init: Setting up sound"), 1);
    S_Init();
    //IO_StartupTimer();
    S_Start();

    tprintf(DEH_String("D_CheckNetGame: Checking network game status"), 1);
    ...
    tprintf(DEH_String("SB_Init: Loading patches"), 1);
    SB_Init();
    IncThermo();
