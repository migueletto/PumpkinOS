...
static int D_GrabMouseCallback(void)
{
...
{
    DG_debug(DEBUG_INFO, "Hexen: Beyond Heretic");
    finish = 1;
}
...
void D_DoomMain(void)
{
    ...
    DG_debug(DEBUG_INFO, "%s", PACKAGE_STRING);
...
#endif
    M_SetConfigDir(NULL);
    M_SetConfigFilenames("hexen.cfg", NULL);
    M_LoadDefaults();
    ...
    if (iwadfile == NULL)
    {
        I_Error("Game mode indeterminate. No IWAD was found");
    }
