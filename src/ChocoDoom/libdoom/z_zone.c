...
// Z_FileDumpHeap
//
void Z_FileDumpHeap (dg_file_t* f)
{
    memblock_t*	block;
	
    DG_printf (f,"zone size: %i  location: %p\n",mainzone->size,mainzone);
	
    for (block = mainzone->blocklist.next ; ; block = block->next)
    {
	DG_printf (f,"block:%p    size:%7i    user:%p    tag:%3i\n",
		 block, block->size, block->user, block->tag);
	...
	if ( (byte *)block + block->size != (byte *)block->next)
	    DG_printf (f,"ERROR: block size does not touch the next block\n");

	if ( block->next->prev != block)
	    DG_printf (f,"ERROR: next block doesn't have proper back link\n");

	if (block->tag == PU_FREE && block->next->tag == PU_FREE)
	    DG_printf (f,"ERROR: two consecutive free blocks\n");
    }
}
