...
static void SaveDefaultCollection(default_collection_t *collection)
{
    ...
    for (i=0 ; i<collection->numdefaults ; i++)
    {
        ...
        chars_written = DG_printf(f, "%s ", defaults[i].name);
        ...
        switch (defaults[i].type) 
        {
            ...
            case DEFAULT_INT:
	        DG_printf(f, "%i", *defaults[i].location.i);
                break;

            case DEFAULT_INT_HEX:
	        DG_printf(f, "0x%x", *defaults[i].location.i);
                break;

            case DEFAULT_FLOAT:
                DG_printf(f, "%f", *defaults[i].location.f);
                break;

            case DEFAULT_STRING:
	        DG_printf(f,"\"%s\"", *defaults[i].location.s);
                break;
        }
...
static void LoadDefaultCollection(default_collection_t *collection)
{
    ...
    while (!DG_eof(f))
    {
        if (DG_fgets(f, line, sizeof(line)) == NULL) break;
...
void M_LoadDefaults (void)
{
	...
	DG_debug(DEBUG_INFO, "default file: %s",doom_defaults.filename);
    }
    else
    {
    ...
    DG_debug(DEBUG_INFO,"saving config in %s", doom_defaults.filename);
    ...
    if (i)
    {
        ...
        DG_debug(DEBUG_INFO,"extra configuration file: %s", 
...
void M_SetConfigDir(const char *dir)
{
    ...
    {
        DG_debug(DEBUG_INFO,"Using %s for configuration and saves", configdir);
    }
...
char *M_GetSaveGameDir(const char *iwadname)
{
    ...
    if (p)
    {
        ...
        DG_debug(DEBUG_INFO,"Save directory changed to %s", savegamedir);
    }
#ifdef _WIN32
