...
static dg_file_t *net_debug = NULL;
...
void NET_Log(const char *fmt, ...)
{
    ...
    DG_printf(net_debug, "%8d: ", I_GetTimeMS());
    va_start(args, fmt);
    sys_vsnprintf(buf, sizeof(buf), fmt, args);
    va_end(args);
    DG_printf(net_debug, "\n");
}
...
void NET_LogPacket(net_packet_t *packet)
{
    ...
    DG_printf(net_debug, "\t%02x", packet->data[packet->pos]);
    for (i = 1; i < bytes; ++i)
    {
        if ((i % 16) == 0)
        {
        ...
        DG_printf(net_debug, "%02x", packet->data[packet->pos + i]);
    }
    DG_printf(net_debug, "\n");
}
