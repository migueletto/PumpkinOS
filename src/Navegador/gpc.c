...
#define FREE(p)            {if (p) {MemPtrFree(p); (p)= NULL;}}
...
{
  COORD_TYPE              x;            /* X coordinate component            */
  COORD_TYPE              y;            /* Y coordinate component            */
...
typedef struct edge_shape
{
  ...
  COORD_TYPE              xb;           /* Scanbeam bottom x coordinate      */
  COORD_TYPE              xt;           /* Scanbeam top x coordinate         */
  COORD_TYPE              dx;           /* Change in x for a unit y increase */
...
{
  COORD_TYPE              y;            /* Y coordinate at local minimum     */
...
{
  COORD_TYPE              y;            /* Scanbeam node y value             */
...
{
  COORD_TYPE             xmin;          /* Minimum x coordinate              */
  COORD_TYPE             ymin;          /* Minimum y coordinate              */
  COORD_TYPE             xmax;          /* Maximum x coordinate              */
  COORD_TYPE             ymax;          /* Maximum y coordinate              */
} bbox;
...
static edge_node **bound_list(lmt_node **lmt, COORD_TYPE y);
static void add_to_sbtree(int *entries, sb_tree **sbtree, COORD_TYPE y);
static void build_sbt(int *entries, COORD_TYPE *sbt, sb_tree *sbtree);
...
static void add_intersection(it_node **it, edge_node *edge0, edge_node *edge1, COORD_TYPE x, COORD_TYPE y);
static void add_st_edge(st_node **st, it_node **it, edge_node *edge, COORD_TYPE dy);
static void build_intersection_table(it_node **it, edge_node *aet, COORD_TYPE dy);
static int count_contours(polygon_node *polygon);
static void add_left(polygon_node *p, COORD_TYPE x, COORD_TYPE y);
static void merge_left(polygon_node *p, polygon_node *q, polygon_node *list);
static void add_right(polygon_node *p, COORD_TYPE x, COORD_TYPE y);
static void merge_right(polygon_node *p, polygon_node *q, polygon_node *list);
static void add_local_min(polygon_node **p, edge_node *edge, COORD_TYPE x, COORD_TYPE y);
...
static bbox *create_contour_bboxes(gpc_polygon *p)
{
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    ...
    box[c].xmin= COORD_MAX;
    box[c].ymin= COORD_MAX;
    box[c].xmax= -COORD_MAX;
    box[c].ymax= -COORD_MAX;
                      ...
                      gpc_polygon *result)
{
  ...
  COORD_TYPE        *sbt= NULL, xb, px, yb, yt, dy, ix, iy;
  ...
  MALLOC(sbt, sbt_entries * sizeof(COORD_TYPE), "sbt creation", COORD_TYPE);
