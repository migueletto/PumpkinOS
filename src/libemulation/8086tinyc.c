...
#endif
#endif
#define KEYBOARD_DRIVER i8086_kbhit(i8086->computer) && (mem[0x4A6] = i8086_getc(i8086->computer), pc_interrupt(7))
...
int i8086_loop(i8086_t *i8086, uint32_t cycles) {
  uint32_t counter;
			...
			OPCODE 48: // Emulator-specific 0F xx opcodes
				switch ((char)i_data0)
				{
					OPCODE_CHAIN 0: // PUTCHAR_AL
						//write(1, regs8, 1)
            i8086_putc(i8086->computer, regs8[0])
					OPCODE 1: // GET_RTC
						//time(&clock_buf);
						//ftime(&ms_clock);
						...
						tm.tm_min = secs / 60;
						tm.tm_sec = secs % 60;
						memcpy(mem + SEGREG(REG_ES, REG_BX,), &tm, sizeof(struct tm));
					...
					OPCODE_CHAIN 3: // DISK_WRITE
						...
						regs8[REG_AL] = ~lseek(disk[regs8[REG_DL]], CAST(unsigned)regs16[REG_BP] << 9, 0)
							...
							: 0;
#endif
            regs8[REG_AL] = i8086_seek(i8086->computer, regs8[REG_DL], CAST(unsigned)regs16[REG_BP]) == 0
              ? (
                  i_data0 == 3
                    ? i8086_write(i8086->computer, regs8[REG_DL], mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX])
                    : i8086_read(i8086->computer, regs8[REG_DL], mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX])
                )
              : 0;
				}
		}
