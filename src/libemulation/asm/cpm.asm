...
;   Set origin for CP/M
;
	ORG	ccp		;ccp is defined in dr.inc
;
...
VERIFY1:LD	A,(DE)
	CP	(HL)
	JP	NZ,DOHALT		;jump to halt routine.
	INC	DE
	INC	HL
	DEC	B
	JP	NZ,VERIFY1
...
;
DOHALT:	LD	HL,$76F3	;'DI HLT' instructions.
	LD	(CBASE),HL
	LD	HL,CBASE
	JP	(HL)
;
...
TYPE:	CALL	CONVFST		;convert file name.
	...
	LD	(HL),$0FF	;set to read first sector.
TYPE1:	LD	HL,NBYTES
...
;
FUNCTNS:DW	WBOOT,GETCON,OUTCON,GETRDR,PUNCH,DOLIST,DIRCIO,GETIOB
...
OUTCHAR:LD	A,(OUTFLAG)	;check output flag.
	...
	OR	A
	CALL	NZ,DOLIST		;print it also if non-zero.
	POP	BC
...
;
	...
	LD	HL,BIGDISK
	LD	(HL),$0FF	;set to samll.
	OR	A
	JP	Z,SELECT1
	LD	(HL),0		;wrong, set to large.
SELECT1:LD	A,$0FF		;clear the zero flag.
	OR	A
	RET	
;
...
STBITMAP: PUSH	DE
	...
	AND	$0FE		;clear the affected bit.
	POP	BC
...
BITMAP2:LD	C,$0FF		;read next file name in directory
...
ERAFIL1:CALL	CKFILPOS	;any found?
	...
	LD	(HL),$0E5	;set first byte to 'empty'.
...
CLOSEIT5: INC	DE		;bump to next item in table.
	...
	LD	BC,$0FFEC	;backup 20 places (extent byte).
	ADD	HL,BC
	EX	DE,HL
	ADD	HL,BC
	LD	A,(DE)
...
CLOSEIT6: LD	A,$0FF		;set 'was open and is now closed' byte.
	LD	(CLOSEFLG),A
...
RDSEQ:	LD	A,1		;set sequential access mode.
	LD	(MODE),A
RDSEQ1:	LD	A,$0FF		;don't allow reading unwritten space.
	LD	(RDWRTFLG),A
...
;
READRAN:LD	C,$0FF		;set 'read' status.
...
;
AUTOSEL:LD	A,$0FF		;say 'auto-select activated'.
	LD	(AUTO),A
	...
	AND	$0E0		;whats this for (bits 6,7 are used for
	LD	(HL),A		;something)?
...
;**************************************************************
;
EMPTYFCB: DB	$0E5		;empty directory segment indicator.
WRTPRT:	DW	0		;write protect status for all 16 drives.
LOGIN:	DW	0		;drive active word (1 bit per drive).
USERDMA:DW	$080		;user's dma address (defaults to 80h).
;
