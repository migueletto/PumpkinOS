...
static void i8086disasm(uint8_t *data, uint32_t ip, char *buf, int len) {
  ...
  if (ZYAN_SUCCESS(ZydisDisassembleIntel( ZYDIS_MACHINE_MODE_REAL_16, runtime_address, data, 16, &instruction))) {
    strncpy(buf, instruction.text, len);
  } else {
    strncpy(buf, "invalid instruction", len);
  }
}
...
fprintf(stderr, "%04X:%04X AX=%04X BX=%04X CX=%04X DX=%04X BP=%04X DS=%04X C%dZ%d %s\n",
