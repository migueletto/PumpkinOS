...
Boolean		SaveBackup = true;						// true if save backup to PC is the default
	...
	if ( err == errNone )
		...
		*outHandleP = recordH;
		...
		if ( apptRec.description != NULL )
			...
			*outOffsetP = (UInt16) ((UIntPtr) apptRec.description - (UIntPtr) apptRec.when); // XXX
	...
	UInt16						inRecordNum )
{
	...
	if (inRecordNum != noAppointments)
		{
		ApptGetRecord (inDB, inRecordNum, &apptRec, &recordH);
