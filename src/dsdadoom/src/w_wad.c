...
// killough 1/31/98: static, const
{
  ...
  DG_debug(DEBUG_INFO,"adding %s",wadfile->name);
  startlump = numlumps;
  ...
  if (  strlen(wadfile->name)<=4 ||
  ...
  else
    {
      ...
      DG_seek(wadfile->handle, header.infotableofs, DG_SEEK_SET),
...
void W_ReadLump(int lump, void *dest)
{
  ...
  if (lump >= numlumps)
...

    {
      if (l->wadfile)
      {
        DG_seek(l->wadfile->handle, l->position, DG_SEEK_SET);
