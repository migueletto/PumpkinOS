...
#undef NO_DIRENT_IMPLEMENTATION
                         ...
                         const char *glob, ...)
{
    ...
    result->dir = DG_opendir(directory_native);
    if (result->dir == NULL)
    {
        FreeStringList(globs, num_globs);
        Z_Free(result);
...
static char *NextGlob(glob_t *glob)
{
    ...
    do
    {
        isdir = DG_readdir(glob->dir, name, sizeof(name));
        if (isdir == -1)
        {
            return NULL;
        }
    } while (isdir || !MatchesAnyGlob(name, glob));

    // Return the fully-qualified path, not just the bare filename.
    len = snprintf(NULL, 0, "%s/%s", glob->directory, name);
    temp = Z_Malloc(len+1);
    snprintf(temp, len+1, "%s/%s", glob->directory, name);
