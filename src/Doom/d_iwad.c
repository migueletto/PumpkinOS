...
static char *CheckDirectoryHasIWAD(char *dir, char *iwadname)
{
    ...
    {
        filename = M_StringJoin(!strcmp(dir, DIR_SEPARATOR_S) ? "" : dir, DIR_SEPARATOR_S, iwadname, NULL);
    }

    DG_debug(1, "Trying IWAD file %s", filename);
...
char *D_FindIWAD(int mask, GameMission_t *mission)
{
    ...
    else
    {
        ...
        DG_debug(1, "-iwad not specified, trying a few iwad names");

        result = NULL;

        drootparm = M_CheckParmWithArgs("-droot", 1);
        if (drootparm) {
          drootdir = myargv[drootparm + 1];
          AddIWADDir(drootdir);
        }
