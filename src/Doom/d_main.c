...
void D_IdentifyVersion(void)
{
    ...
    if (gamemission == none)
    {
        ...
        {
            if (!xstrncasecmp(lumpinfo[i].name, "MAP01", 8))
            {
                gamemission = doom2;
                break;
            } 
            else if (!xstrncasecmp(lumpinfo[i].name, "E1M1", 8))
            {
                gamemission = doom;
                break;
            }
...
static wad_file_t *D_AddFile(char *filename)
{
    wad_file_t *handle;

    //printf(" adding %s\n", filename);
    DG_debug(1, "adding %s", filename);
...
void PrintGameVersion(void)
{
    ...
    for (i=0; gameversions[i].description != NULL; ++i)
    {
        if (gameversions[i].version == gameversion)
        {
            ...
            DG_debug(1, "Emulating the behavior of the '%s' executable", gameversions[i].description);
            break;
        }
    }
}
...
void D_DoomMain (void)
{
    ...
    DG_debug(1, "Init zone memory allocation daemon");
    Z_Init ();
    ...
    if (M_CheckParm("-dedicated") > 0)
    {
        ...
        DG_debug(1, "Dedicated server mode");
        NET_DedicatedServer();
...
#endif
    {
        ...
        int drootparm = M_CheckParmWithArgs("-droot", 1);
        if (drootparm) {
          char *drootdir = myargv[drootparm + 1];
          M_SetConfigDir(drootdir);
        }
    }
    ...
    DG_debug(1, "allocate screens");
    V_Init ();

    // Load configuration files before initialising other subsystems.
    //DEH_printf("M_LoadDefaults: Load system defaults.\n");
    DG_debug(1, "Load system defaults");
    ...
    if (iwadfile == NULL)
    {
        I_Error("No WAD file was found.");
    }

    modifiedgame = false;

    //DEH_printf("W_Init: Init WADfiles.\n");
    DG_debug(1, "Init WADfiles");
    wf = D_AddFile(iwadfile);
#if ORIGCODE
    ...
    if (W_CheckNumForName("dmenupic") >= 0)
    {
        ...
        DG_debug(1, "BFG Edition: Using workarounds as needed");
        bfgedition = true;
...
//    W_PrintDirectory();
    ...
    DG_debug(1, "Setting up machine state");
    ...
    DG_debug(1, "Init miscellaneous info");
    M_Init ();

    //DEH_printf("R_Init: Init DOOM refresh daemon - ");
    DG_debug(1, "Init DOOM refresh daemon");
    R_Init ();

    //DEH_printf("\nP_Init: Init Playloop state.\n");
    DG_debug(1, "Init Playloop state");
    P_Init ();

    //DEH_printf("S_Init: Setting up sound.\n");
    DG_debug(1, "Setting up sound");
    S_Init (sfxVolume * 8, musicVolume * 8);

    //DEH_printf("D_CheckNetGame: Checking network game status.\n");
    DG_debug(1, "Checking network game status");
    D_CheckNetGame ();

    PrintGameVersion();

    //DEH_printf("HU_Init: Setting up heads up display.\n");
    DG_debug(1, "Setting up heads up display");
    HU_Init ();

    //DEH_printf("ST_Init: Init status bar.\n");
    DG_debug(1, "Init status bar");
    ST_Init ();
    ...
    if (M_CheckParmWithArgs("-statdump", 1))
    {
        ...
        DG_debug(1, "External statistics registered");
    }
