...
void M_LoadDefaults (void)
{
    ...
    DG_debug(1, "saving config in %s", doom_defaults.filename);
    ...
    if (i)
    {
        ...
        DG_debug(1, "extra configuration file: %s", extra_defaults.filename);
    }
    else
    {
        extra_defaults.filename
...
void M_SetConfigDir(char *dir)
{
    ...
    if (strcmp(configdir, "") != 0)
    {
        ...
        DG_debug(1, "Using %s for configuration and saves", configdir);
    }
...
char *M_GetSaveGameDir(char *iwadname)
{
    ...
    else
    {
        ...
        free(topdir);
#else
        savegamedir = M_StringJoin(configdir, DIR_SEPARATOR_S, "savegame", DIR_SEPARATOR_S, NULL);
        ...
        DG_debug(1, "Using %s for savegames", savegamedir);
#endif
    }
