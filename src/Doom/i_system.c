...
byte *I_ZoneBase (int *size)
{
    ...
    DG_debug(1, "zone memory: %p, %x allocated for zone", zonemem, *size);
...
*/
    DG_debug(1, PACKAGE_NAME " is free software, covered by the GNU General Public");
    DG_debug(1, "License. There is NO warranty; not even for MERCHANTABILITY or FITNESS");
    DG_debug(1, "FOR A PARTICULAR PURPOSE. You are welcome to change and distribute");
    DG_debug(1, "copies under certain conditions. See the source for more information.");
...
void I_Error (char *error, ...)
{
    ...
    if (already_quitting)
    {
        DG_debug(0, "Warning: recursive call to I_Error detected");
#if ORIGCODE
        exit(-1);
#endif
    }
