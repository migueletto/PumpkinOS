...
void G_DoLoadGame (void) 
{
    ...
    save_stream = DG_open(savename, 0);
...
{ 
    ...
    DG_debug(1, "temporary save file \"%s\"", temp_savegame_file);

    // Open the savegame file for writing.  We write to a temporary file
    // and then rename it at the end if it was successfully written.
    // This prevents an existing savegame from being overwritten by 
    // a corrupted one, or if a savegame buffer overrun occurs.
    if ((save_stream = DG_open(temp_savegame_file, 1)) == NULL) {
        DG_create(temp_savegame_file);
        if ((save_stream = DG_open(temp_savegame_file, 1)) == NULL) {
    ...
    if (vanilla_savegame_limit && DG_tell(save_stream) > SAVEGAMESIZE)
    {
    ...
    DG_remove(savegame_file);
    DG_rename(temp_savegame_file, savegame_file);
    DG_debug(1, "final save file \"%s\"", savegame_file);
