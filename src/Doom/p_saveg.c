...
static byte saveg_read8(void) {
  ...
  if (DG_read(save_stream, &result, 1) != 1) {
    if (!savegame_error) {
      DG_debug(0, "saveg_read8: Unexpected end of file while eading save game");
      savegame_error = true;
    }
  }
...
static void saveg_write8(byte value) {
  if (DG_write(save_stream, &value, 1) != 1) {
    if (!savegame_error) {
      DG_debug(0, "saveg_write8: Error while writing save game");
      savegame_error = true;
    }
  }
}
