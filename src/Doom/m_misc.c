...
long M_FileLength(FILE *handle) { 
  DG_debug(0, "M_FileLength called");
  return 0;
}
*/

boolean M_WriteFile(char *name, void *source, int length) {
  DG_debug(0, "M_WriteFile \"%s\" called", name);
  return 0;
}
...
/*
    ...
    *buffer = buf;
    return length;
*/
  DG_debug(0, "M_ReadFile \"%s\" called", name);
  return 0;
}
...
char *M_StrCaseStr(char *haystack, char *needle)
{
    ...
    for (i = 0; i <= len; ++i)
    {
        if (!xstrncasecmp(haystack + i, needle, needle_len))
        {
            return haystack + i;
        }
    }
