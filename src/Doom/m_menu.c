...
int			detailLevel = 0;
//int			screenblocks = 10;
int			screenblocks = 11; // libgdoom: hides status bar
...
{
    {1,"M_JKILL",	"Kill",      M_ChooseSkill, 'i'},
    {1,"M_ROUGH",	"Rough",     M_ChooseSkill, 'h'},
    {1,"M_HURT",	"Hurt",      M_ChooseSkill, 'h'},
    {1,"M_ULTRA",	"Ultra",     M_ChooseSkill, 'u'},
...
menuitem_t OptionsMenu[]=
{
    ...
    {1,"M_DETAIL",	"Detail", M_ChangeDetail,'g'},
    {2,"M_SCRNSZ",	"Display", M_SizeDisplay,'s'},
    {-1,"","", 0,'\0'},
    {2,"M_MSENS",	  "Sensitivity", M_ChangeSensitivity,'m'},
    {-1,"","", 0,'\0'},
    {1,"M_SVOL",	  "Sound", M_Sound,'s'}
};
...
void M_ReadSaveStrings(void) {
  ...
  for (i = 0; i < load_end; i++) {
    ...
    if (f == NULL) {
	  ...
	  DG_read(f, &savegamestrings[i], SAVESTRINGSIZE);
	  DG_close(f);
	  LoadMenu[i].status = 1;
  }
}
...
void M_Episode(int choice) {
  ...
  if ((gamemode == registered) && (choice > 2)) {
    DG_debug(1, "M_Episode: 4th episode requires UltimateDOOM");
    choice = 0;
  }
...
*/
    ...
    for (i=0;i<max;i++)
    {
        name = DEH_String(currentMenu->menuitems[i].descr);
        if (!name[0]) name = DEH_String(currentMenu->menuitems[i].name);

	if (name[0])
	{
      char aux[32];
      aux[0] = (i == itemOn) ? '*' : ' ';
      strncpy(aux+1, name, sizeof(aux)-2);
