...
void D_CheckNetGame (void)
{
    ...
    DG_debug(1, "startskill %i  deathmatch %i  startmap %i  startepisode %i",
      startskill, deathmatch, startmap, startepisode);

    //DEH_printf("player %i of %i (%i nodes)\n",
               //consoleplayer+1, settings.num_players, settings.num_players);
    DG_debug(1, "player %i of %i (%i nodes)",
      consoleplayer+1, settings.num_players, settings.num_players);
    ...
    if (timelimit > 0 && deathmatch)
    {
        ...
        if (timelimit == 20 && M_CheckParm("-avg"))
        {
            ...
            DG_debug(1, "Austin Virtual Gaming: Levels will end after 20 minutes");
        }
        else
        {
            //DEH_printf("Levels will end after %d minute", timelimit);
            DG_debug(1, "Levels will end after %d minute(s)", timelimit);
            //if (timelimit > 1)
                //printf("s");
            //printf(".\n");
        }
