...
#define GPIO_MIN_BCM_PIN 0
#define GPIO_MAX_BCM_PIN 53

#define MUTEX_GPIO "mgpio"

struct gpio_t {
  int pin_direction[GPIO_MAX_BCM_PIN+1];
};
...
int gpio_output_multi(gpio_t *gpio, int n, int *pins, uint32_t values) {
  ...
  for (i = 0; i < n; i++) {
    if ((r = gpio_output(gpio, pins[i], values & 1)) == -1) break;
    values >>= 1;
  }
