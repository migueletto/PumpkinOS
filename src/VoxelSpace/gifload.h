...
static void load_gif_frame( void* data, struct GIF_WHDR* whdr ) {
  ...
  MemMove( context->palette, whdr->cpal, whdr->clrs * 3 );
  context->pixels = (uint8_t*)malloc( whdr->xdim * whdr->ydim );
  MemSet( context->pixels, whdr->xdim * whdr->ydim, 0 );
...
unsigned char* load_gifm(uint8_t *data, size_t sz, int* palcount, unsigned char palette[768]) {
  struct gif_load_context_t context;
