...
static void fillObjectNames(form_edit_t *data) {
  ...
  for (index = 0; index < data->formP->numObjects; index++) {
    ...
    switch (FrmGetObjectType(data->formP, index)) {
      case frmFieldObj:
        StrNPrintF(buf, sizeof(buf)-1, "Field %d", obj.field->id);
        break;
      case frmControlObj:
        StrNPrintF(buf, sizeof(buf)-1, "Control %d", obj.control->id);
        break;
      case frmListObj:
        StrNPrintF(buf, sizeof(buf)-1, "List %d", obj.list->id);
        break;
      case frmTitleObj:
        StrNPrintF(buf, sizeof(buf)-1, "Title");
        break;
      case frmTableObj:
        StrNPrintF(buf, sizeof(buf)-1, "Table %d", obj.table->id);
        break;
      case frmBitmapObj:
        StrNPrintF(buf, sizeof(buf)-1, "Bitmap %d", obj.bitmap->rscID);
        break;
      case frmLineObj:
        StrNPrintF(buf, sizeof(buf)-1, "Line");
        break;
      case frmFrameObj:
        StrNPrintF(buf, sizeof(buf)-1, "Frame");
        break;
      case frmRectangleObj:
        StrNPrintF(buf, sizeof(buf)-1, "Rect");
        break;
      case frmLabelObj:
        StrNPrintF(buf, sizeof(buf)-1, "Label %d", obj.label->id);
        break;
      case frmPopupObj:
        StrNPrintF(buf, sizeof(buf)-1, "Popup %d %d", obj.popup->controlID, obj.popup->listID);
        break;
      case frmGraffitiStateObj:
        StrNPrintF(buf, sizeof(buf)-1, "Graffiti");
        break;
      case frmGadgetObj:
...
static void titlePropertiesCallback(FormType *frm, dynamic_form_phase_t phase, void *_data) {
  ...
  switch (phase) {
    case setProperties:
      text = (char *)FrmObjectGetField(title, frmTitleObj, FormTitleFieldText);
      setField(frm, 1000, text, true);
      break;
    case getProperties:
      getField(frm, 1000, buf, sizeof(buf));
      len = StrLen(buf);
      old = (char *)FrmObjectGetField(title, frmTitleObj, FormTitleFieldText);
      text = MemPtrNew(len + 1);
      FrmObjectSetField(title, frmTitleObj, FormTitleFieldText, (UIntPtr)text);
      MemMove(text, buf, len);
  ...
  switch (event->eType) {
    case frmOpenEvent:
      ...
      if (data->formP->numObjects > 0) {
        ...
        ctl = (ControlType *)FrmGetObjectPtr(frm, index);
        CtlSetLabel(ctl, s);
        StrNPrintF(data->title, MAX_TITLE-1, "%s: %s", data->rtitle, s);
      } else {
        index = FrmGetObjectIndex(frm, objectCtl);
