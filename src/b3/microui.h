...
typedef struct { mu_BaseCommand base; mu_Font font; mu_Vec2 pos; mu_Color color; mu_Color back; char str[1]; } mu_TextCommand;
...
struct mu_Context {
  /* callbacks */
  int (*text_width)(mu_Font font, char *str, int len);
...
void mu_draw_text(mu_Context *ctx, mu_Font font, char *str, int len, mu_Vec2 pos, mu_Color color, mu_Color back);
...
void mu_draw_control_text(mu_Context *ctx, char *str, mu_Rect rect, int colorid, int opt);
int mu_mouse_over(mu_Context *ctx, mu_Rect rect);
void mu_update_control(mu_Context *ctx, mu_Id id, mu_Rect rect, int opt);

#define mu_button(ctx, label)             mu_button_ex(ctx, label, 0, MU_OPT_ALIGNCENTER)
#define mu_link(ctx, label)               mu_link_ex(ctx, label, 0)
...
int mu_slider_ex(mu_Context *ctx, mu_Real *value, mu_Real low, mu_Real high, mu_Real step, char *fmt, int opt);
int mu_number_ex(mu_Context *ctx, mu_Real *value, mu_Real step, char *fmt, int opt);
