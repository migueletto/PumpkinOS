...
static void element_destroy_paths(element_t *e) {
    element_t *child = e->first;
    while (child) {
        element_destroy_paths(child);
        child = child->next;
    }
    if (e->path0) {
      plutovg_path_destroy(e->path0);
    }
    if (e->path) {
      plutovg_path_destroy(e->path);
    }
}

/*
static void element_print(element_t *e) {
    debug(1, "XXX", "element %s %p", elementmap[e->id].name, e);
    element_t *child = e->first;
    debug_indent(2);
    while (child) {
        element_print(child);
...
static plutovg_document_t* parse_document(const char* data, int size)
{
    const char* ptr = data;
    ...
    plutovg_document_t* document = malloc(sizeof(plutovg_document_t));
    memset(document, 0, sizeof(plutovg_document_t));
    document->root = root;
...
static void document_destroy(plutovg_document_t* document)
{
    element_destroy_paths(document->root);
    hashmap_destroy(document->cache);
    heap_destroy(document->heap);
    free(document);
}

static const string_t* findvalue(element_t * e, int id)
{
...
static int parse_number(element_t * e, int id, double* number, int percent)
{
...
static int parse_length(element_t * e, int id, length_t* length, int negative)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_transform_list(element_t * e, int id, plutovg_matrix_t* matrix)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_path(element_t * e, int id, plutovg_path_t* path)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_points(element_t * e, int id, plutovg_path_t* path, int closed)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_positioning(element_t * e, int id, positioning_t* position)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_line_cap(element_t * e, int id, plutovg_line_cap_t* cap)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_line_join(element_t * e, int id, plutovg_line_join_t* join)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_fill_rule(element_t * e, int id, plutovg_fill_rule_t* winding)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_spread_method(element_t * e, int id, plutovg_spread_method_t* spread)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_display(element_t * e, int id, int* display)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_visibility(element_t * e, int id, int* visibility)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_units_type(element_t * e, int id, int* units)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static int parse_text_anchor(element_t * e, int id, int* anchor)
{
    const string_t* value = findvalue(e, id);
    if(value == NULL)
        return 0;
...
static void parse_style(element_t *e, style_t *style) {
...
static render_state_t *render_state_clone(const render_state_t *state) {
    ...
    newstate->canvas = state->canvas ? canvas_reference(state->canvas) : NULL;
...
static void render_state_destroy(render_state_t *state) {
    if (state->canvas) canvas_destroy(state->canvas);
    free(state);
}
...
static render_context_t *render_context_create(plutovg_document_t *document, plutovg_font_t *font, double dpi) {
    render_context_t *context = malloc(sizeof(render_context_t));
    context->document = document;
    context->state = render_state_create();
    context->font = font ? plutovg_font_reference(font) : NULL;
    context->dpi = dpi;
    return context;
}

static void render_context_destroy(render_context_t *context) {
    if (context->state) render_state_destroy(context->state);
    if (context->font) plutovg_font_destroy(context->font);
    free(context);
}

static void render_context_push(render_context_t *context, element_t *e, heap_t *heap) {
  ...
  if (context->render == 0) {
    if (!parse_transform_list(e, ID_TRANSFORM, &e->matrix))
      plutovg_matrix_init_identity(&e->matrix);
  } else {
    plutovg_matrix_multiply(&newstate->matrix, &e->matrix, &newstate->matrix);
  }

  paint_t paint = { .type = paint_type_none };
  if (context->render == 0) {
    parse_paint(e, ID_COLOR, &paint);
    e->paint = heap_alloc(heap, sizeof(paint_t));
    memcpy(e->paint, &paint, sizeof(paint_t));
  }
  memcpy(&paint, e->paint, sizeof(paint_t));
...
static void render_context_pop(render_context_t *context, element_t *e) {
...
static element_t * resolve_iri(const render_context_t* context, const string_t* value)
{
    const char* ptr = value->start;
    const char* end = value->end;
    ...
    if(size > 1 && *ptr == '#')
    {
        const plutovg_document_t* document = context->document;
...
static element_t * resolve_href(const render_context_t* context, element_t * e)
{
...
static plutovg_color_t find_current_color(element_t * e)
{
    paint_t paint = {.type=paint_type_none};
    parse_paint(e, ID_COLOR, &paint);
    if(paint.type == paint_type_color)
...
static plutovg_color_t find_solid_color(element_t * e, int id)
{
    paint_t paint = {.type=paint_type_none};
...
static double find_solid_opacity(element_t * e, int id)
{
    double opacity;
...
static plutovg_paint_t* resolve_solid_color(element_t * e)
{
...
static element_t * findgradient(const render_context_t* context, element_t * e)
{
    element_t * ref = resolve_href(context, e);
...
static int parse_gradient_length(const render_context_t* context, element_t * e, int id, length_t* length, int negative)
{
    if(parse_length(e, id, length, negative))
        return 1;

    element_t * ref = findgradient(context, e);
    if(ref == NULL)
        return 0;
...
static element_t * find_gradient_stops(const render_context_t* context, element_t * e)
{
    element_t * child = e->first;
...
static plutovg_spread_method_t find_gradient_spread(const render_context_t* context, element_t * e)
{
...
static plutovg_matrix_t find_gradient_matrix(const render_context_t* context, element_t * e)
{
    plutovg_matrix_t matrix;
...
static int find_gradient_units(const render_context_t* context, element_t * e)
{
    int units;
...
static void resolve_gradient_stops(plutovg_gradient_t* gradient, element_t * e)
{
    while(e != NULL)
    {
        if(e->id == TAG_STOP)
...
static plutovg_paint_t* resolve_linear_gradient(const render_context_t* context, element_t * e)
{
    element_t * stops = find_gradient_stops(context, e);
    if(stops == NULL)
        return NULL;
...
static plutovg_paint_t* resolve_radial_gradient(const render_context_t* context, element_t * e)
{
    element_t * stops = find_gradient_stops(context, e);
    if(stops == NULL)
        return NULL;
...
static plutovg_paint_t* resolve_paint(const render_context_t* context, const paint_t* paint)
{
    ...
    element_t * ref = resolve_iri(context, &paint->url);
    if(ref == NULL)
        return NULL;
...
static void render_context_draw(render_context_t *context, plutovg_path_t *path) {
  render_state_t *state = context->state;
  style_t *style = &state->style;
...
static int element_display(element_t *e) {
  int display = display_inline;
  parse_display(e, ID_DISPLAY, &display);
  return display;
}

static void render_element(render_context_t *context, element_t *e, heap_t *heap);
static void render_children(render_context_t *context, element_t *e, heap_t *heap);

static void render_symbol(render_context_t *context, element_t *e, double x, double y, double w, double h, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);

    if (parse_view_box(e, ID_VIEW_BOX, &e->data.symbol.viewbox)) {
      e->data.symbol.parsed = 1;
      e->data.symbol.position.align = positioning_align_x_mid_y_mid;
      e->data.symbol.position.scale = positioning_scale_meet;
      parse_positioning(e, ID_PRESERVE_ASPECT_RATIO, &e->data.symbol.position);
    }

    render_context_push(context, e, heap);
    render_children(context, e, heap);
    render_context_pop(context, e);

  } else if (e->display != display_none) {
    render_context_push(context, e, heap);
    ...
    if (e->data.symbol.parsed) {
      plutovg_matrix_t matrix;
      positioning_get_matrix(&e->data.symbol.position, &matrix, &state->viewport, &e->data.symbol.viewbox);
      plutovg_matrix_multiply(&state->matrix, &matrix, &state->matrix);
      state->viewport = e->data.symbol.viewbox;
    } else {
...
static void render_svg(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);
    ...
    e->data.svg.x = resolve_length(context, &x, 'x');
    e->data.svg.y = resolve_length(context, &y, 'y');
    e->data.svg.w = resolve_length(context, &w, 'x');
    e->data.svg.h = resolve_length(context, &h, 'y');

    render_symbol(context, e, e->data.svg.x, e->data.svg.y, e->data.svg.w, e->data.svg.h, heap);
...
static void render_use(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);

    e->data.use.ref = resolve_href(context, e);
    if (e->data.use.ref == NULL)
      return;
    ...
    e->data.use.x = resolve_length(context, &x, 'x');
    e->data.use.y = resolve_length(context, &y, 'y');
    e->data.use.w = resolve_length(context, &w, 'x');
    e->data.use.h = resolve_length(context, &h, 'y');

  } else if (e->display != display_none) {
    render_context_push(context, e, heap);
    plutovg_matrix_translate(&context->state->matrix, e->data.use.x, e->data.use.y);

    if (e->data.use.ref->id == TAG_SVG || e->data.use.ref->id == TAG_SYMBOL) {
      render_symbol(context, e->data.use.ref, 0, 0, e->data.use.w, e->data.use.h, heap);
    } else {
      render_element(context, e->data.use.ref, heap);
    }
...
static void render_g(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);
    render_context_push(context, e, heap);
    render_children(context, e, heap);
...
static void render_line(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);
    ...
    e->data.line.x1 = resolve_length(context, &x1, 'x');
    e->data.line.y1 = resolve_length(context, &y1, 'y');
    e->data.line.x2 = resolve_length(context, &x2, 'x');
    e->data.line.y2 = resolve_length(context, &y2, 'y');

    e->path0 = plutovg_path_create();
    plutovg_path_clear(e->path0);
    plutovg_path_move_to(e->path0, e->data.line.x1, e->data.line.y1);
    plutovg_path_line_to(e->path0, e->data.line.x2, e->data.line.y2);
    e->path = plutovg_path_clone(e->path0);

  } else if (e->display != display_none) {
    render_context_push(context, e, heap);

    render_state_t *state = context->state;
    state->bbox.x = MIN(e->data.line.x1, e->data.line.x2);
    state->bbox.y = MIN(e->data.line.y1, e->data.line.y2);
    state->bbox.w = fabs(e->data.line.x2 - e->data.line.x1);
    state->bbox.h = fabs(e->data.line.y2 - e->data.line.y1);

    render_context_draw(context, e->path);
    render_context_pop(context, e);
  }
}

static void render_polyline(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);

    e->path0 = plutovg_path_create();
    plutovg_path_clear(e->path0);
    parse_points(e, ID_POINTS, e->path0, 0);
    e->path = plutovg_path_clone(e->path0);

  } else if (e->display != display_none) {
    render_context_push(context, e, heap);
    render_state_t *state = context->state;
    path_bounding_box(e->path, &state->bbox);
    render_context_draw(context, e->path);
    render_context_pop(context, e);
  }
}

static void render_polygon(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);

    e->path0 = plutovg_path_create();
    plutovg_path_clear(e->path0);
    parse_points(e, ID_POINTS, e->path0, 1);
    e->path = plutovg_path_clone(e->path0);
...
static void render_path(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);

    e->path0 = plutovg_path_create();
    plutovg_path_clear(e->path0);
    parse_path(e, ID_D, e->path0);
    e->path = plutovg_path_clone(e->path0);
...
static void render_ellipse(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);
    ...
    e->data.ellipse.cx = resolve_length(context, &cx, 'x');
    e->data.ellipse.cy = resolve_length(context, &cy, 'y');
    e->data.ellipse.rx = resolve_length(context, &rx, 'x');
    e->data.ellipse.ry = resolve_length(context, &ry, 'y');

    e->path0 = plutovg_path_create();
    plutovg_path_clear(e->path0);
    plutovg_path_add_ellipse(e->path0, e->data.ellipse.cx, e->data.ellipse.cy, e->data.ellipse.rx, e->data.ellipse.ry);
    e->path = plutovg_path_clone(e->path0);
    ...
    render_state_t *state = context->state;
    state->bbox.x = e->data.ellipse.cx - e->data.ellipse.rx;
    state->bbox.y = e->data.ellipse.cy - e->data.ellipse.ry;
    state->bbox.w = e->data.ellipse.rx + e->data.ellipse.rx;
    state->bbox.h = e->data.ellipse.ry + e->data.ellipse.ry;
...
static void render_circle(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);
    ...
    e->data.circle.cx = resolve_length(context, &cx, 'x');
    e->data.circle.cy = resolve_length(context, &cy, 'y');
    e->data.circle.r = resolve_length(context, &r, 'o');

    e->path0 = plutovg_path_create();
    plutovg_path_clear(e->path0);
    plutovg_path_add_circle(e->path0, e->data.circle.cx, e->data.circle.cy, e->data.circle.r);
    e->path = plutovg_path_clone(e->path0);
    ...
    render_state_t *state = context->state;
    state->bbox.x = e->data.circle.cx - e->data.circle.r;
    state->bbox.y = e->data.circle.cy - e->data.circle.r;
    state->bbox.w = e->data.circle.r + e->data.circle.r;
    state->bbox.h = e->data.circle.r + e->data.circle.r;
...
static void render_rect(render_context_t *context, element_t *e, heap_t *heap) {
  if (context->render == 0) {
    e->display = element_display(e);
    ...
    e->data.rect.x = resolve_length(context, &x, 'x');
    e->data.rect.y = resolve_length(context, &y, 'y');
    e->data.rect.w = resolve_length(context, &w, 'x');
    e->data.rect.h = resolve_length(context, &h, 'y');
    ...
    e->path0 = plutovg_path_create();
    plutovg_path_clear(e->path0);
    plutovg_path_add_round_rect(e->path0, e->data.rect.x, e->data.rect.y, e->data.rect.w, e->data.rect.h, _rx, _ry);
    e->path = plutovg_path_clone(e->path0);
    ...
    render_state_t *state = context->state;
    state->bbox.x = e->data.rect.x;
    state->bbox.y = e->data.rect.y;
    state->bbox.w = e->data.rect.w;
    state->bbox.h = e->data.rect.h;
...
static void render_element(render_context_t *context, element_t *e, heap_t *heap) {
  switch (e->id) {
    case TAG_SVG:
      render_svg(context, e, heap);
      break;
    case TAG_USE:
      render_use(context, e, heap);
      break;
    case TAG_G:
      render_g(context, e, heap);
      break;
    case TAG_LINE:
      render_line(context, e, heap);
      break;
    case TAG_POLYLINE:
      render_polyline(context, e, heap);
      break;
    case TAG_POLYGON:
      render_polygon(context, e, heap);
      break;
    case TAG_PATH:
      render_path(context, e, heap);
      break;
    case TAG_ELLIPSE:
      render_ellipse(context, e, heap);
      break;
    case TAG_CIRCLE:
      render_circle(context, e, heap);
      break;
    case TAG_RECT:
      render_rect(context, e, heap);
      break;
  }
}
...
static void render_children(render_context_t *context, element_t *e, heap_t *heap) {
  element_t *child = e->first;
  while (child) {
    render_element(context, child, heap);
    child = child->next;
  }
}

/*
static void document_dimensions(plutovg_document_t* document, plutovg_font_t* font, double* width, double* height, double dpi) {
  ...
  *height = length_value(&h, 1.0, dpi);
  ...
  if (*width == 0.0 || *height == 0.0) {
    ...
    render_element(context, document->root, NULL);
...
plutovg_document_t *plutosvg_parse_from_memory(const char *data, int size) {
  return parse_document(data, size);
}
...
int plutosvg_document_prepare(plutovg_document_t *document, double dpi) {
  render_context_t *context = render_context_create(document, NULL, dpi);
  context->render = 0;
  render_element(context, document->root, document->heap);
  render_context_destroy(context);

  document->w = document->root->data.svg.w;
  document->h = document->root->data.svg.h;

  return 0;
}

static void plutosvg_element_transform(element_t *e, plutovg_matrix_t *matrix, int freeze) {
  if (e->path) {
    if (matrix) {
      // transform
      plutovg_path_transform(e->path, matrix);
    } else if (freeze) {
      // freeze
      plutovg_path_t *old = e->path0;
      e->path0 = plutovg_path_clone(e->path);
      plutovg_path_destroy(old);
    } else {
      // reset
      plutovg_path_t *old = e->path;
      e->path = plutovg_path_clone(e->path0);
      plutovg_path_destroy(old);
    }
  }
  ...
  element_t *child = e->first;
  while (child) {
    plutosvg_element_transform(child, matrix, 0);
    child = child->next;
  }
}

static void plutosvg_document_transform(plutovg_document_t *document, plutovg_matrix_t *matrix, int freeze) {
  plutosvg_element_transform(document->root, matrix, freeze);
}

void plutosvg_document_reset(plutovg_document_t *document) {
  plutosvg_document_transform(document, NULL, 0);
}
...
void plutosvg_document_rotate(plutovg_document_t *document, double radians) {
  ...
  plutovg_matrix_init_identity(&matrix);
  plutovg_matrix_rotate(&matrix, radians);
  plutosvg_document_transform(document, &matrix, 0);
...
void plutosvg_document_scale(plutovg_document_t *document, double factor) {
  ...
  plutovg_matrix_init_identity(&matrix);
  plutovg_matrix_scale(&matrix, factor, factor);
  plutosvg_document_transform(document, &matrix, 0);
}
...
int plutosvg_document_render(plutovg_document_t *document, plutovg_surface_t *surface, plutovg_font_t *font, double dpi) {
  ...
  context->render = 1;
  render_state_t *state = context->state;
  plutovg_matrix_scale(&state->matrix, width / document->w, height / document->h);
  state->canvas = canvas_create_for_surface(surface);
  state->viewport.w = document->w;
  state->viewport.h = document->h;
...
{
    plutovg_document_t* document = parse_document(data, size);
    if(document == NULL)
        return NULL;
