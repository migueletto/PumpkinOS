...
//#define apptTimeFont 					FossilStdFont
//#define apptEmptyDescFont				FossilStdFont
//#define noteTitleFont					FossilBoldFont
...
void GoToItem (GoToParamsPtr goToParams, Boolean launchingApp) EXTRA_SECTION_TWO;

void GoToAlarmItem (UInt32 uniqueID) EXTRA_SECTION_TWO;

//extern void* GetObjectPtr (UInt16 objectID);
void* GetObjectPtr (UInt16 objectID) EXTRA_SECTION_TWO;
...
void DrawTime (
	...
	RectangleType*					inBounds ) EXTRA_SECTION_TWO;

void ShowInfo (void) EXTRA_SECTION_TWO;

void DirtyRecord (DmOpenRef dbP, UInt16 index) EXTRA_SECTION_TWO;

Err MoveEvent (UInt16* recordNumP, TimeType startTime, TimeType endTime,
 		DateType startDate, Boolean timeChangeOnly, Boolean* moved) EXTRA_SECTION_TWO;

Int16 DatebookLoadPrefs (DatebookPreferenceType* prefsP) EXTRA_SECTION_TWO;

void DatebookSavePrefs (void) EXTRA_SECTION_TWO;

Int32 TimeToWait (void) EXTRA_SECTION_TWO;

Char* DateParamString(const Char* inTemplate, const Char* param0,
			const Char* param1, const Char* param2, const Char* param3) EXTRA_SECTION_TWO;

void DoSecurity (void ) EXTRA_SECTION_TWO;

Boolean ClearEditState (void) EXTRA_SECTION_TWO;
