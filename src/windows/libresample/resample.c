...
void *resample_dup(const void *handle)
{
   ...
   rsdata *hp = (rsdata *)sys_malloc(sizeof(rsdata));
   ...
   sys_memcpy(hp->Imp, cpy->Imp, hp->Nwing * sizeof(float));
   hp->ImpD = (float *)sys_malloc(hp->Nwing * sizeof(float));
   sys_memcpy(hp->ImpD, cpy->ImpD, hp->Nwing * sizeof(float));

   hp->Xoff = cpy->Xoff;
   hp->XSize = cpy->XSize;
   hp->X = (float *)sys_malloc((hp->XSize + hp->Xoff) * sizeof(float));
   sys_memcpy(hp->X, cpy->X, (hp->XSize + hp->Xoff) * sizeof(float));
   hp->Xp = cpy->Xp;
   hp->Xread = cpy->Xread;
   hp->YSize = cpy->YSize;
   hp->Y = (float *)sys_malloc(hp->YSize * sizeof(float));
   sys_memcpy(hp->Y, cpy->Y, hp->YSize * sizeof(float));
   hp->Yp = cpy->Yp;
   hp->Time = cpy->Time;
...
void *resample_open(int highQuality, double minFactor, double maxFactor)
{
   ...
   if (minFactor <= 0.0 || maxFactor <= 0.0 || maxFactor < minFactor) {
      debug(DEBUG_ERROR, "resample",
        "minFactor and maxFactor must be positive real numbers and maxFactor should be larger than minFactor");
      return 0;
   }

   hp = (rsdata *)sys_malloc(sizeof(rsdata));
   ...
   Imp64 = (double *)sys_malloc(hp->Nwing * sizeof(double));
                     ...
                     int     outBufferLen)
{
   ...
   debug(DEBUG_TRACE, "resample", "resample_process: in=%d, out=%d lastFlag=%d", inBufferLen, outBufferLen, lastFlag);
   ...
   *inBufferUsed = 0;
   ...
   if (factor < hp->minFactor || factor > hp->maxFactor) {
      debug(DEBUG_ERROR, "resample", "factor %f is not between " "minFactor=%f and maxFactor=%f",
   ...
   for(;;) {
      ...
      *inBufferUsed += len;
      ...
      debug(DEBUG_TRACE, "resample", "new len=%d Nx=%d", len, Nx);
      ...
      if (Nout > hp->YSize) {
         debug(DEBUG_ERROR, "resample", "output array overflow");
         return -1;
      }
