...
static LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (uMsg) {
    ...
    case WM_PAINT:
      ...
      debug(DEBUG_TRACE, "Windows", "paint %ld %ld %ld %ld", ps.rcPaint.left, ps.rcPaint.right, ps.rcPaint.top, ps.rcPaint.bottom);
...
static int window_draw_texture_rect(window_t *_window, texture_t *texture, int tx, int ty, int w, int h, int x, int y) {
  ...
  if (texture && window->bitmapBuffer) {
    ...
    if (w > 0 && h > 0) {
      ...
      rect.top = y;
      rect.bottom = y + h;
      debug(DEBUG_TRACE, "Windows", "invalidate %ld %ld %ld %ld", rect.left, rect.right, rect.top, rect.bottom);
