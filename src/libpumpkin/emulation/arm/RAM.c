...
static int ramAccessF(void* userData, uint32_t pa, uint8_t size, int write, void* bufP)
{
	...
	if (write) {
		...
		switch (size) {
			case 1:
        			debug(DEBUG_TRACE, "logmem", "read %u %u", pa, 1);
				*(uint8_t*)bufP = *((uint8_t*)addr);
				break;
			case 2:
        			debug(DEBUG_TRACE, "logmem", "read %u %u", pa, 2);
				*(uint16_t*)bufP = sys_le16toh(*((uint16_t*)addr));
				break;
			case 4:
        			debug(DEBUG_TRACE, "logmem", "read %u %u", pa, 4);
				*(uint32_t*)bufP = sys_le32toh(*((uint32_t*)addr));
				break;
			case 8:
        			debug(DEBUG_TRACE, "logmem", "read %u %u", pa, 8);
				((uint32_t*)bufP)[0] = sys_le32toh(*((uint32_t*)(addr +  0)));
				((uint32_t*)bufP)[1] = sys_le32toh(*((uint32_t*)(addr +  4)));
				break;
			case 16:
        			debug(DEBUG_TRACE, "logmem", "read %u %u", pa, 16);
				((uint32_t*)bufP)[0] = sys_le32toh(*((uint32_t*)(addr +  0)));
				((uint32_t*)bufP)[1] = sys_le32toh(*((uint32_t*)(addr +  4)));
				...
				break;
			case 32:
        			debug(DEBUG_TRACE, "logmem", "read %u %u", pa, 32);
				((uint32_t*)bufP)[0] = sys_le32toh(*((uint32_t*)(addr +  0)));
				((uint32_t*)bufP)[1] = sys_le32toh(*((uint32_t*)(addr +  4)));
				((uint32_t*)bufP)[2] = sys_le32toh(*((uint32_t*)(addr +  8)));
				((uint32_t*)bufP)[3] = sys_le32toh(*((uint32_t*)(addr + 12)));
				...
				break;
			case 64:
        			debug(DEBUG_TRACE, "logmem", "read %u %u", pa, 64);
				((uint32_t*)bufP)[0] = sys_le32toh(*((uint32_t*)(addr +  0)));
				((uint32_t*)bufP)[1] = sys_le32toh(*((uint32_t*)(addr +  4)));
				((uint32_t*)bufP)[2] = sys_le32toh(*((uint32_t*)(addr +  8)));
				((uint32_t*)bufP)[3] = sys_le32toh(*((uint32_t*)(addr + 12)));
