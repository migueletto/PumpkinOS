  ...
  if (dbP) {
    ...
    if (mutex_lock(module->dm->mutex) == 0) {
      if (db->ftype == DATAMGR_TYPE_REC) {
        if (index < db->numRecs) {
          if (uniqueIDP) {
            db->idxRec[index].uniqueID = *uniqueIDP;
            if (db->idxRec[index].h) db->idxRec[index].h->d.rec.uniqueID = *uniqueIDP;
          }
          if (attrP) {
            db->idxRec[index].attr = *attrP; 
            if (db->idxRec[index].h) db->idxRec[index].h->d.rec.attr = *uniqueIDP;
          }
          err = errNone;
        }
      }
