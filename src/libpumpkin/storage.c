...
void *StoNewDecodedResource(void *h, UInt32 size, DmResType resType, DmResID resID) {
  ...
  if (size) {
    ...
    if (h != NULL) {
      h2 = h;
      // bitmaps created with pumpkin_create_bitmap() are faked into a resource handle and
      // have the standard bitmap destructor. Since dmRecAttached bit is not set, they can be freed.
      ...
      if (resType == bitmapRsc) {
        h2->d.res.destructor = pumpkin_destroy_bitmap;
        h2->d.res.encoder = pumpkin_encode_bitmap;
        h2->d.res.decoded = p;
      }
  ...
  if (res && !res->d.res.decoded) {
    ...
    switch (res->d.res.type) {
      ...
      case 'tRAW':  // XXXX sigh.. FreeJongg stores bitmaps as tRAW
        ...
        if ((p = pumpkin_create_bitmap(res, res->buf, res->size, res->d.res.type, 1, &dsize)) != NULL) {
          res->d.res.destructor = pumpkin_destroy_bitmap;
          res->d.res.encoder = pumpkin_encode_bitmap;
          res->d.res.decoded = p;
          res->d.res.decodedSize = dsize;
          BmpPrintChain((BitmapType *)p, res->d.res.type, res->d.res.id, "decode");
        }
        break;
...
UInt32 MemHeapSize(UInt16 heapID) {
  UInt32 size;
  size = heapID == 0 ? 1*1024*1024 : pumpkin_heap_size();
  return size;
}
