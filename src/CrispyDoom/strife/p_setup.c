...
{
    int i;
    const int rightangle = iabs(finesine[(ANG60/2) >> ANGLETOFINESHIFT]);
    ...
    for (i = 0; i < numsegs; i++)
    {
        ...
        if (!contrast_only)
        {
            li->length = (uint32_t)(sys_sqrt((double)dx*dx + (double)dy*dy)/2);
        ...
        if (iabs(finesine[li->r_angle >> ANGLETOFINESHIFT]) < rightangle)
            li->fakecontrast = -(LIGHTBRIGHT >> 1);
        else
        if (!dx)
            li->fakecontrast = LIGHTBRIGHT;
        else
        if (iabs(finecosine[li->r_angle >> ANGLETOFINESHIFT]) < rightangle)
            li->fakecontrast = LIGHTBRIGHT >> 1;
        else
            li->fakecontrast = 0;
    }
...
static void P_RemoveSlimeTrails(void)
{
	...
	if (l->dx && l->dy)
	{
		...
		if (!v->moved)
		{
			...
			if (iabs(v->r_x - v->x) > 8*FRACUNIT || iabs(v->r_y - v->y) > 8*FRACUNIT)
			{
			    v->r_x = v->x;
			    v->r_y = v->y;
			}
