...
boolean W_ParseCommandLine(void)
{
    ...
    if (p > 0)
    {
        for (p = p + 1; p<myargc && myargv[p][0] != '-'; ++p)
        {
            ...
            DG_debug(DEBUG_INFO, "merging %s", filename);
    ...
    if (p > 0)
    {
        for (p = p + 1; p<myargc && myargv[p][0] != '-'; ++p)
        {
            ...
            DG_debug(DEBUG_INFO, "merging flats from %s", filename);
    ...
    if (p > 0)
    {
        for (p = p + 1; p<myargc && myargv[p][0] != '-'; ++p)
        {
            ...
            DG_debug(DEBUG_INFO, "merging sprites from %s", filename);
    ...
    if (p > 0)
    {
        for (p = p + 1; p<myargc && myargv[p][0] != '-'; ++p)
        {
            ...
            DG_debug(DEBUG_INFO, "merging sprites and flats from %s", filename);
...
void W_AutoLoadWADs(const char *path)
{
    ...
    for (;;)
    {
        ...
        DG_debug(DEBUG_INFO, "[autoload] merging %s", filename);
        W_MergeFile(filename);
    }
