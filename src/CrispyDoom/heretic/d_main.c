...
boolean D_AddFile(char *file)
{
    ...
    DG_debug(DEBUG_INFO, "adding %s", file);
...
//
    ...
    DG_debug(DEBUG_INFO, DEH_String("MN_Init: Init menu system"));
    MN_Init();

    CT_Init();

    DG_debug(DEBUG_INFO, DEH_String("R_Init: Init Heretic refresh daemon"));
    DG_debug(DEBUG_INFO, DEH_String("Loading graphics"));
    R_Init();

    DG_debug(DEBUG_INFO, DEH_String("P_Init: Init Playloop state"));
    DG_debug(DEBUG_INFO, DEH_String("Init game engine"));
    P_Init();
    IncThermo();

    DG_debug(DEBUG_INFO, DEH_String("I_Init: Setting up machine state"));
    //I_CheckIsScreensaver();
    //I_InitJoystick();
    IncThermo();

    DG_debug(DEBUG_INFO, DEH_String("S_Init: Setting up sound"));
    S_Init();
    //IO_StartupTimer();
    S_Start();

    DG_debug(DEBUG_INFO, DEH_String("D_CheckNetGame: Checking network game status"));
    DG_debug(DEBUG_INFO, DEH_String("Checking network game status"));
    D_CheckNetGame();
    IncThermo();

    // haleyjd: removed WATCOMC

    DG_debug(DEBUG_INFO, DEH_String("SB_Init: Loading patches"));
    SB_Init();
    IncThermo();
