...
void SV_Close(char *fileName)
{
    ...
    if (vanilla_savegame_limit && DG_tell(SaveGameFP) > SAVEGAMESIZE)
    {
...
{
    DG_write(SaveGameFP, buffer, size);
}
...
{
    int retval = DG_read(SaveGameFP, buffer, size);
    if (retval != size)
    {
