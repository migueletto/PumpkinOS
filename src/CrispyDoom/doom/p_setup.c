...
fixed_t GetOffset(vertex_t *v1, vertex_t *v2)
{
    ...
    r = (fixed_t)(sys_sqrt(dx*dx + dy*dy))<<FRACBITS;
...
void P_LoadNodes (int lump)
{
    ...
    {
	if (numsubsectors == 1)
	    DG_debug(DEBUG_INFO, "P_LoadNodes: No nodes in map, but only one subsector");
	else
...
*/
	...
	DG_debug(DEBUG_INFO, "P_SetupLevel: %s (%s) %s%s %d:%02d:%02d/%d:%02d:%02d",
