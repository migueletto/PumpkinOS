...
void D_DoomLoop (void)
{
    ...
    {
        D_RunFrame();
        if (players[displayplayer].mo && players[displayplayer].mo->subsector && players[displayplayer].mo->subsector->sector) {
          DG_sectorTag(players[displayplayer].mo->subsector->sector->tag);
        }
    }
}
...
void PrintGameVersion(void)
{
    ...
    for (i=0; gameversions[i].description != NULL; ++i)
    {
        ...
        {
            DG_debug(DEBUG_INFO, "Emulating the behavior of the "
...
void D_DoomMain (void)
{
    ...
    DG_debug(DEBUG_INFO, "Z_Init: Init zone memory allocation daemon");
    Z_Init ();
    ...
    // init subsystems
    DG_debug(DEBUG_INFO, "V_Init: allocate screens");
    V_Init ();

    // Load configuration files before initialising other subsystems.
    DG_debug(DEBUG_INFO, "M_LoadDefaults: Load system defaults");
    M_SetConfigFilenames("default.cfg", "extra.cfg");
    D_BindVariables();
    M_LoadDefaults();
    ...
    DG_debug(DEBUG_INFO, "W_Init: Init WADfiles");
    D_AddFile(iwadfile);
...
//    W_PrintDirectory();
    ...
    if (!M_ParmExists("-nodehlump") && !M_ParmExists("-nodeh"))
    {
        ...
        DG_debug(DEBUG_INFO, "loaded %i DEHACKED lumps from PWAD files", loaded);
    }
...
// [crispy] disable meaningless warning, we always use "-merge" anyway
    ...
    DG_debug(DEBUG_INFO, "I_Init: Setting up machine state");
    ...
    DG_debug(DEBUG_INFO, "M_Init: Init miscellaneous info");
    M_Init ();

    DG_debug(DEBUG_INFO, "R_Init: Init DOOM refresh daemon");
    R_Init ();

    DG_debug(DEBUG_INFO, "P_Init: Init Playloop state");
    P_Init ();

    DG_debug(DEBUG_INFO, "S_Init: Setting up sound");
    S_Init (sfxVolume * 8, musicVolume * 8);

    DG_debug(DEBUG_INFO, "D_CheckNetGame: Checking network game status");
    D_CheckNetGame ();

    PrintGameVersion();

    DG_debug(DEBUG_INFO, "HU_Init: Setting up heads up display");
    HU_Init ();

    DG_debug(DEBUG_INFO, "ST_Init: Init status bar");
    ST_Init ();
