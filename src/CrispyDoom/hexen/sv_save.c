...
void SV_LoadGame(int slot)
{
    ...
    DG_seek(SavingFP, HXS_DESCRIPTION_LENGTH);
...
static void CopyFile(char *source_name, char *dest_name)
{
    ...
    dg_file_t *read_handle, *write_handle;
    ...
    do
    {
        ...
        read_count = DG_read(read_handle, buffer, buf_count);
        if (read_count < buf_count)
        {
            I_Error ("Couldn't read file %s", source_name);
        }

        write_count = DG_write(write_handle, buffer, buf_count);
    ...
    Z_Free(buffer);
    DG_close(read_handle);
    DG_close(write_handle);
}
...
static void SV_Read(void *buffer, int size)
{
    int retval = DG_read(SavingFP, buffer, size);
    if (retval != size)
    {
...
static void SV_Write(const void *buffer, int size)
{
    DG_write(SavingFP, (void *)buffer, size);
}

static void SV_WriteByte(byte val)
{
    DG_write(SavingFP, &val, sizeof(byte));
}

static void SV_WriteWord(unsigned short val)
{
    val = SHORT(val);
    DG_write(SavingFP, &val, sizeof(unsigned short));
}

static void SV_WriteLong(unsigned int val)
{
    val = LONG(val);
    DG_write(SavingFP, &val, sizeof(int));
}
